...
function renderProducts() {
  ...
  // Sincronizar corazones de la Wishlist tras renderizar los productos
  if (window.syncWishlistVisuals) {
    window.syncWishlistVisuals();
  }
}
...
function renderProductCard(item, catId = '') {
  ...
  const wishlistBtnHtml = !isRegalo ? `
    <button class="wishlist-btn" data-product-id="${item.id}" onclick="window.handleWishlistClick(event, '${item.id}')" aria-label="Guardar en wishlist">
  ...
  // Inject wishlist button inside special widgets (.thumb wrapper)
  if (['pase-corporativo', 'gift-card-25', 'gift-card-50'].includes(item.id) && wishlistBtnHtml) {
    mediaHtml = mediaHtml.trim().replace(/<\/div>$/, `${wishlistBtnHtml}</div>`);
  }
...
function closeAll() {
  ...
  const wishlistModal = document.getElementById('wishlist-register-modal');
...
// --- WISHLIST HANDLERS & DRAWER RENDERING ---
  ...
  if (wishlist.length === 0) {
    container.innerHTML = `
      <div class="wishlist-empty-state">
        ...
        <p style="font-size: 12px; margin-top: 10px; opacity: 0.7;">Presiona el ❤️ en las imágenes del menú para guardar tus platos favoritos aquí.</p>
      </div>
    `;
    return;
  }
        ...
        <div class="wishlist-item">
          ...
          <div class="wishlist-item-info">
            ...
            <div class="wishlist-item-actions">
              <button class="add-to-cart-btn" onclick="handleAddToCart('${item.id}', '${catId}'); window.toggleWishlistPanel();">Agregar +</button>
              <button class="remove-item-btn" onclick="window.toggleWishlistItem('${item.id}')">Eliminar</button>
            </div>
          </div>
        </div>
      `;
